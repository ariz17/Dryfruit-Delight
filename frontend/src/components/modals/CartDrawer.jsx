      ...
      <div className="bg-white dark:bg-zinc-950 w-full max-w-md h-full flex flex-col shadow-2xl border-l border-zinc-200 dark:border-zinc-800 animate-in slide-in-from-right duration-300 text-zinc-900 dark:text-zinc-100">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          {cart.length === 0 ? (
            <div className="text-center py-16 space-y-3 text-zinc-500">
              ...
              <button onClick={closeModal} className="bg-zinc-900 text-white dark:bg-white dark:text-black text-xs font-semibold px-4 py-2 rounded-lg mt-2">
                Browse Catalog
              </button>
            </div>
          ) : (
                ...
                <div key={item._id} className="bg-zinc-50 dark:bg-zinc-900/50 border border-zinc-200 dark:border-zinc-800 rounded-xl p-3 flex gap-3 items-center">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      <button onClick={() => updateCartQuantity(item._id, item.quantity - 1)} className="w-5 h-5 rounded bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 flex items-center justify-center text-zinc-700 dark:text-zinc-300 hover:bg-zinc-100 text-xs">
                      ...
                      <button onClick={() => removeFromCart(item._id)} className="text-zinc-400 hover:text-zinc-900 dark:hover:text-white ml-auto p-1" title="Remove">
        ...
        {cart.length > 0 && (
          <div className="border-t border-zinc-200 dark:border-zinc-800 p-4 bg-zinc-50 dark:bg-zinc-900/60 space-y-3">
            ...
            >
              {orderComplete ? (
                <><CheckCircle2 className="w-4 h-4 text-emerald-400 dark:text-emerald-600" /><span>Order Placed! Generating Tax PDF...</span></>
              ) : checkingOut ? (
                <span>Applying Apollo Subsidy & Finalizing...</span>
              ) : (
                <><span>Pay ₹{finalPayable} (With Subsidy)</span><ArrowRight className="w-4 h-4" /></>
              )}
            </button>
          </div>
        )}
