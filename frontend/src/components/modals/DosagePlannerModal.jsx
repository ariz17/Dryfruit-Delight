...
import { X, Check, HeartPulse, ShieldCheck, ArrowRight } from 'lucide-react';
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="bg-zinc-50 dark:bg-zinc-900/60 border border-zinc-200 dark:border-zinc-800 rounded-lg p-3.5 flex items-start justify-between gap-3">
            ...
            <span className="text-[10px] bg-zinc-900 text-white dark:bg-white dark:text-black font-bold px-2 py-0.5 rounded flex items-center gap-1">
              <ShieldCheck className="w-3 h-3 text-emerald-400 dark:text-emerald-600" /> Apollo Approved
            </span>
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="space-y-2">
              {guide.schedule.map((item, idx) => (
                <div key={idx} className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg p-2.5 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                  <div className="flex items-start gap-2.5">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-zinc-600 dark:text-zinc-400">{item.count} • <span className="text-zinc-400">{item.prep}</span></p>
                    </div>
                  </div>
          ...
          <div className="bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg p-3 flex items-start gap-2.5 text-xs text-zinc-700 dark:text-zinc-300">
            ...
            <p className="text-[11px] leading-relaxed"><strong>Clinical Note:</strong> {guide.apolloNote}</p>
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-3 pt-2 border-t border-zinc-200 dark:border-zinc-800">
            <div className="text-[11px] text-zinc-500">Pre-portioned vacuum sealed supply for {selectedCondition}</div>
            <button
              ...
              className={`w-full sm:w-auto px-4 py-2 rounded-lg font-bold text-xs flex items-center justify-center gap-1.5 transition ${
                added ? 'bg-emerald-600 text-white' : 'bg-zinc-900 hover:bg-black text-white dark:bg-white dark:text-black dark:hover:bg-zinc-200 shadow-xs'
              }`}
            >
              {added ? (
                <><Check className="w-3.5 h-3.5" /><span>Added to Cart!</span></>
              ) : (
                <><span>Add Recommended 30-Day Pack (₹1,299)</span><ArrowRight className="w-3.5 h-3.5" /></>
              )}
            </button>
          </div>
        </div>
