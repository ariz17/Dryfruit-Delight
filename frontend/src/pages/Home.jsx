...
import { ShieldCheck, HeartPulse, CheckCircle2, FileText, X, ShoppingCart, Clock, Check, ArrowRight, User } from 'lucide-react';
import Sidebar from '../components/Sidebar';
import { useStore, SAMPLE_PROFILES } from '../store/useStore';
    ...
    <div className="bg-white dark:bg-zinc-950 rounded-2xl border border-zinc-200 dark:border-zinc-800 overflow-hidden shadow-sm hover:shadow-md hover:border-zinc-400 dark:hover:border-zinc-600 transition-all duration-200 flex flex-col">
      ...
      <div className="p-4 flex-1 space-y-2">
        <Link to={`/product/${product._id}`}>
          <h3 className="text-sm font-bold text-zinc-900 dark:text-white hover:underline leading-tight">{product.name}</h3>
        </Link>
        <p className="text-sm text-zinc-500 dark:text-zinc-400 leading-relaxed line-clamp-2">{product.description}</p>
      ...
      <div className="px-4 pb-4 pt-3 border-t border-zinc-100 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/40 space-y-3">
        <div className="flex items-end justify-between">
          <div>
            ...
            <span className="text-xs font-semibold text-green-600 dark:text-green-400">Save ₹{savings} ({subsidyPercent}% Apollo subsidy)</span>
          </div>
        ...
        <div className="grid grid-cols-2 gap-2">
          <Link to={`/product/${product._id}`} className="text-center text-sm font-semibold py-2 bg-white dark:bg-zinc-800 hover:bg-zinc-50 dark:hover:bg-zinc-700 border border-zinc-200 dark:border-zinc-700 text-zinc-700 dark:text-zinc-200 rounded-xl transition">
            Details
          </Link>
          <button onClick={() => onAddToCart(product)} className="text-center text-sm font-bold py-2 bg-zinc-900 hover:bg-black dark:bg-white dark:text-black dark:hover:bg-zinc-100 text-white rounded-xl transition flex items-center justify-center gap-1.5">
...
export default function Home() {
  ...
  const {
    profile, setProfile, selectSampleProfile, subsidyPercent, prescriptionDoc,
    isVerifying, verifyDocument, selectedCategory, setSelectedCategory,
    selectedHealthNeed, setSelectedHealthNeed, searchQuery, setSearchQuery,
    addToCart, openModal, activeSection, setActiveSection
  } = useStore();
  ...
  useEffect(() => {
    fetch('http://localhost:5000/api/products')
      ...
      .then((data) => {
        setProducts(data.map((item) => ({ ...(PRODUCTS.find((p) => p._id === String(item._id)) || {}), ...item })));
      })
  ...
  const handleSaveProfile = (e) => {
    e.preventDefault();
    setProfile({ ...profileForm, age: Number(profileForm.age) });
  ...
  const filteredProducts = products.filter((p) => {
    ...
    const needMatch = selectedHealthNeed === 'All' || (p.healthNeeds && p.healthNeeds.includes(selectedHealthNeed));
    const searchMatch = !query || p.name.toLowerCase().includes(query) || p.category.toLowerCase().includes(query) || p.description.toLowerCase().includes(query) || (p.healthNeeds && p.healthNeeds.some((h) => h.toLowerCase().includes(query)));
    return catMatch && needMatch && searchMatch;
  });
  ...
  return (
    <div className="max-w-7xl mx-auto">
      {toastMessage && (
        <div className="fixed bottom-6 right-6 z-50 bg-zinc-900 dark:bg-white text-white dark:text-black px-5 py-3 rounded-2xl shadow-xl flex items-center gap-2.5 text-sm font-semibold border border-zinc-700 dark:border-zinc-300">
            ...
            <div className="space-y-8">
              <div className="bg-white dark:bg-zinc-950 rounded-2xl border border-zinc-200 dark:border-zinc-800 p-8 shadow-sm">
                <div className="mb-4 flex flex-wrap gap-2 items-center">
                  <span className="bg-zinc-900 dark:bg-white text-white dark:text-black text-xs font-bold px-3 py-1 rounded-full flex items-center gap-1.5 uppercase tracking-wider">
                    <span className="w-1.5 h-1.5 rounded-full bg-green-400"></span> Apollo Hospital Partner
                  </span>
                  <span className="border border-zinc-200 dark:border-zinc-700 text-zinc-500 text-xs px-3 py-1 rounded-full">100% Whole Nuts · Zero Additives</span>
                </div>
                <h1 className="text-3xl sm:text-4xl font-extrabold text-zinc-900 dark:text-white leading-tight mb-4">
                  Clinical Dry Fruit Nutrition,<br />Subsidized for Senior Wellness.
                </h1>
                ...
                <div className="flex flex-wrap gap-3">
                  <button onClick={() => setActiveSection('dryfruits')} className="bg-zinc-900 hover:bg-black dark:bg-white dark:text-black dark:hover:bg-zinc-100 text-white font-bold text-sm px-5 py-2.5 rounded-xl transition flex items-center gap-2">
                    Browse All 10 Varieties <ArrowRight className="w-4 h-4" />
                  </button>
                  <button onClick={() => setActiveSection('health')} className="bg-zinc-100 hover:bg-zinc-200 dark:bg-zinc-900 dark:hover:bg-zinc-800 text-zinc-800 dark:text-zinc-200 border border-zinc-200 dark:border-zinc-700 font-semibold text-sm px-5 py-2.5 rounded-xl transition flex items-center gap-2">
                    <HeartPulse className="w-4 h-4 text-zinc-500" /> Health & Subsidy
                  </button>
                  <button onClick={() => setActiveSection('profile')} className="bg-zinc-100 hover:bg-zinc-200 dark:bg-zinc-900 dark:hover:bg-zinc-800 text-zinc-800 dark:text-zinc-200 border border-zinc-200 dark:border-zinc-700 font-semibold text-sm px-5 py-2.5 rounded-xl transition flex items-center gap-2">
              ...
              <div>
                <div className="flex items-center justify-between mb-5">
                  ...
                  <button onClick={() => setActiveSection('dryfruits')} className="text-sm font-semibold text-zinc-700 dark:text-zinc-300 hover:text-zinc-900 dark:hover:text-white flex items-center gap-1 transition">
                    View All (10) <ArrowRight className="w-4 h-4" />
                  </button>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                  {filteredProducts.slice(0, 3).map((p) => (
                    <ProductCard key={p._id} product={p} subsidyPercent={subsidyPercent} onAddToCart={(item) => { addToCart(item, 1); showToast(`Added ${item.name} to cart!`); }} />
                  ))}
                </div>
              </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-5">
                  ...
                  <button onClick={() => setActiveSection('features')} className="text-sm font-semibold text-zinc-700 dark:text-zinc-300 hover:text-zinc-900 dark:hover:text-white transition">See All →</button>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {TOOLS.map((tool) => (
                    <div key={tool.modal} onClick={() => openModal(tool.modal)} className="bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-5 hover:border-zinc-400 dark:hover:border-zinc-600 hover:shadow-md cursor-pointer transition-all duration-200 flex items-start gap-4">
          ...
          {activeSection === 'dryfruits' && (
            <div className="space-y-6">
              <div className="bg-white dark:bg-zinc-950 p-6 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                ...
                {(selectedCategory !== 'All' || selectedHealthNeed !== 'All' || searchQuery !== '') && (
                  <button onClick={clearAllFilters} className="text-sm text-zinc-600 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-white font-medium flex items-center gap-1.5 bg-zinc-100 dark:bg-zinc-800 px-4 py-2 rounded-xl transition self-start sm:self-auto">
              ...
              {filteredProducts.length === 0 ? (
                <div className="bg-white dark:bg-zinc-950 rounded-2xl p-16 text-center space-y-4 border border-zinc-200 dark:border-zinc-800 shadow-sm">
                  ...
                  <button onClick={clearAllFilters} className="bg-zinc-900 text-white dark:bg-white dark:text-black text-sm font-semibold px-6 py-2.5 rounded-xl mt-2">Reset Filters</button>
                </div>
              ) : (
            ...
            <div className="space-y-6">
              <div className="bg-white dark:bg-zinc-950 rounded-2xl border border-zinc-200 dark:border-zinc-800 p-6 shadow-sm">
                <div className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-zinc-500 dark:text-zinc-400 mb-3">
                  <span className="w-2 h-2 rounded-full bg-green-500 block"></span> Apollo EHR Healthcare Program
                </div>
              ...
              <div className="bg-white dark:bg-zinc-950 rounded-2xl border border-zinc-200 dark:border-zinc-800 p-6 shadow-sm space-y-5">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  ...
                  <div className="flex items-center gap-3">
                    <span className="bg-zinc-100 dark:bg-zinc-800 text-zinc-800 dark:text-zinc-200 px-3 py-1.5 rounded-lg text-sm font-bold flex items-center gap-1.5">
                      <CheckCircle2 className="w-4 h-4 text-green-500" /> {subsidyPercent}% Active
                    </span>
                    <button onClick={handleLoadSampleRx} className="text-sm bg-zinc-100 hover:bg-zinc-200 dark:bg-zinc-900 dark:hover:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 text-zinc-700 dark:text-zinc-300 font-semibold px-4 py-2 rounded-xl transition">
                      Load Sample Rx
                    </button>
                  </div>
                </div>
                ...
                <form onSubmit={handleVerify} className="space-y-4">
                  ...
                  <div>
                    ...
                    <input type="file" accept=".pdf,.png,.jpg,.jpeg" onChange={(e) => setLocalDocName(e.target.files[0] ? e.target.files[0].name : '')} className="w-full text-sm text-zinc-500 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-sm file:font-semibold file:bg-zinc-200 dark:file:bg-zinc-800 file:text-zinc-800 dark:file:text-zinc-200 cursor-pointer" />
                    {localDocName && (
                      <p className="text-sm text-zinc-500 dark:text-zinc-400 mt-2 flex items-center gap-2">
                        <FileText className="w-4 h-4" /> {localDocName}
                      </p>
                    )}
                  </div>

                  <button type="submit" disabled={isVerifying} className="w-full bg-zinc-900 hover:bg-black dark:bg-white dark:text-black dark:hover:bg-zinc-100 text-white font-bold py-3 px-6 rounded-xl text-sm transition flex items-center justify-center gap-2">
                    {isVerifying ? (
                      <span>Verifying with Apollo EHR Network...</span>
                    ) : (
                      <><ShieldCheck className="w-4 h-4" /><span>Verify & Apply {subsidyPercent}% Subsidy</span></>
                    )}
                  </button>
                </form>
              </div>
              ...
              <div className="bg-white dark:bg-zinc-950 rounded-2xl border border-zinc-200 dark:border-zinc-800 p-6 shadow-sm space-y-4">
                <h3 className="text-lg font-bold text-zinc-900 dark:text-white flex items-center gap-2">
                  <HeartPulse className="w-5 h-5 text-zinc-500" /> Apollo Clinical Nutrition Guidelines
                </h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {GUIDELINES.map((g) => (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  {SAMPLE_PROFILES.map((p) => {
                    ...
                    return (
                      <button
                        key={p.id}
                        onClick={() => { selectSampleProfile(p.id); showToast(`Switched to ${p.name}`); }}
                        className={`p-4 rounded-xl border text-left transition-all duration-150 ${
                          isSelected ? 'border-zinc-900 dark:border-white bg-zinc-100 dark:bg-zinc-900' : 'border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 hover:border-zinc-400 dark:hover:border-zinc-600'
                        }`}
                      >
                ...
                <form onSubmit={handleSaveProfile} className="space-y-5">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input type="text" value={profileForm.name} onChange={(e) => setProfileForm({ ...profileForm, name: e.target.value })} className={inputCls} required />
                    </div>
                    <div>
                      <label className={labelCls}>Age (Years)</label>
                      <input type="number" min="1" max="120" value={profileForm.age} onChange={(e) => setProfileForm({ ...profileForm, age: e.target.value })} className={inputCls} required />
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input type="text" value={profileForm.uhid} onChange={(e) => setProfileForm({ ...profileForm, uhid: e.target.value })} className={`${inputCls} font-mono`} required />
                    </div>
                    <div>
                      <label className={labelCls}>Blood Group</label>
                      <select value={profileForm.bloodGroup} onChange={(e) => setProfileForm({ ...profileForm, bloodGroup: e.target.value })} className={inputCls}>
                        {['O+', 'A+', 'B+', 'AB+', 'O-', 'A-'].map((bg) => (
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      ...
                      <select value={profileForm.condition} onChange={(e) => setProfileForm({ ...profileForm, condition: e.target.value })} className={inputCls}>
                        <option value="Senior Citizen Care">Senior Care (60+)</option>
                    ...
                    <div>
                      ...
                      <input type="text" value={profileForm.contact} onChange={(e) => setProfileForm({ ...profileForm, contact: e.target.value })} className={inputCls} />
                    </div>
                  </div>

                  <div className="flex justify-end pt-2">
                    <button type="submit" className="bg-zinc-900 hover:bg-black dark:bg-white dark:text-black dark:hover:bg-zinc-100 text-white font-bold text-sm py-2.5 px-6 rounded-xl transition flex items-center gap-2">
          ...
          {activeSection === 'features' && (
            <div className="space-y-6">
              <div className="bg-white dark:bg-zinc-950 rounded-2xl border border-zinc-200 dark:border-zinc-800 p-6 shadow-sm">
                ...
                <p className="text-base text-zinc-500 mt-2">Interactive clinical tools tailored for elderly dietary balance, digestion, tax invoicing, and auto-delivery.</p>
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                {[
                  { emoji: '🥗', title: 'Elderly Dosage Planner', desc: 'Calculates daily grams and exact count of almonds, walnuts, figs, and seeds customized to your chronic condition.', action: 'Open Dosage Planner', modal: 'dosage' },
                  { emoji: '🥣', title: 'Senior Soaking & Digestion Guide', desc: 'Essential overnight soaking protocols to remove tannin acid and tenderize nuts for seniors with dental bridges or dentures.', action: 'Read Soaking Guide', modal: 'soaking' },
                  { emoji: '📄', title: 'Mediclaim & Section 80D Invoice', desc: 'Generate official Apollo Hospital-stamped GST receipts formatted for medical reimbursement and income tax deductions.', action: 'Generate Medical PDF', modal: 'invoice' },
                  { emoji: '🔄', title: 'Monthly Senior Auto-Refill', desc: 'Subscribe to automatic fresh vacuum pack shipments on the 1st of every month with zero delivery charges.', action: 'Set Up Auto-Refill', modal: 'monthlyRefill' }
                ].map((tool) => (
                  <div key={tool.modal} className="bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-6 shadow-sm hover:shadow-md hover:border-zinc-400 dark:hover:border-zinc-600 transition-all duration-200 flex flex-col justify-between space-y-5">
                    <div className="space-y-3">
                    ...
                    <button onClick={() => openModal(tool.modal)} className="self-start bg-zinc-900 hover:bg-black dark:bg-white dark:text-black dark:hover:bg-zinc-100 text-white font-semibold text-sm py-2.5 px-5 rounded-xl transition">
                      {tool.action} →
                    </button>
                  </div>
                ))}
